...
function pushState(row) {
    states.push(row.join("\t"));
    console.log(row.join("\t"));
}

function runInstructions(row) {
    var instructions = 1;

    for (var i=0; i < row.length; i++) {
        if (row[i][0] === "(") {
            var jump = Number(row[i].slice(1, -1));
            var dist = i + jump;
...
while (blocks && counter < 1000 * 1000) {
    // pushState(blocks);
    var result = runInstructions(blocks);

    counter += result.instructions;
