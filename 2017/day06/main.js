...
var blocks = input.split(/\s+/).map(s => Number(s));

var states = {};

function pushState(row, cur_count) {
    var state = row.join("-");

    states[state] = cur_count;
}

function diff(row, count) {
    var state = row.join("-");
...
function newState(row) {
    ...
    for (var j=1; j <= max; j++) {
        var index = (maxIndex+j) % row.length;
...
while (counter < 1000 * 1000) {
    if (hasSeenState(blocks)) {
        break;
    }

    pushState(blocks, counter);
...
console.log("Cycles since configuration", diff(blocks, counter));
